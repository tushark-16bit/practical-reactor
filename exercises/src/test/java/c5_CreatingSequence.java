...
public class c5_CreatingSequence {
    ...
    public void value_I_already_have_mono() {
        ...
        Mono<String> valueIAlreadyHaveMono = Mono.just(valueIAlreadyHave); //todo: change
        // this
        // line
        // only
    ...
    public void potentially_null_mono() {
        ...
        Mono<String> potentiallyNullMono = Mono.justOrEmpty(potentiallyNull); //todo change
        // this line only
    ...
    public void optional_value() {
        ...
        Mono<String> optionalMono = Mono.justOrEmpty(optionalValue); //todo: change this
        // line only
    ...
    public void callable_counter() {
        ...
        Mono<Integer> callableCounterMono = Mono.fromCallable(callable); //todo:
        // change this line
        // only
    ...
    public void future_counter() {
        ...
        Mono<Integer> futureCounterMono = Mono.fromFuture(completableFuture); //todo: change
        // this line only
    ...
    public void runnable_counter() {
        ...
        Mono<Integer> runnableMono = Mono.fromRunnable(runnable); //todo: change
        // this line only
    ...
    public void trouble_maker() {
        Mono<String> trouble = Mono.error(new IllegalStateException()); //todo:
        // change this line
    ...
    public void from_array() {
        ...
        Flux<Integer> arrayFlux = Flux.fromArray(array); //todo: change this
        // line only
    ...
    public void from_list() {
        ...
        Flux<String> listFlux = Flux.fromIterable(list); //todo: change this
        // line
        // only
    ...
    public void from_stream() {
        ...
        Flux<String> streamFlux = Flux.fromStream(stream); //todo: change this
        // line only
    ...
    public void interval() {
        Flux<Long> interval = Flux.interval(Duration.ofSeconds(1)); //todo:
    ...
    public void repeat() {
        ...
        Flux<Integer> repeated = Mono.fromCallable(callable).repeat(9L);
        //todo: change
        // this line
    ...
    public void generate_programmatically() {
        ...
        // generate value each time
        AtomicInteger value = new AtomicInteger(0);
        Flux<Integer> generateFlux = Flux.generate(sink -> {
            value.incrementAndGet();
            if(value.get()<=5) sink.next(value.get());
            else sink.complete();
        ...
        // from synchronous or asynchronous
        Flux<Integer> createFlux = Flux.create(sink -> {
            for(int i =1; i<= 5; i++) {
                 sink.next(i);
            }
            sink.complete();
            //todo: fix following code so it emits
                // values from 0 to 5 and
            // then completes
        });

        //------------------------------------------------------

        // from synchronous multi value call
